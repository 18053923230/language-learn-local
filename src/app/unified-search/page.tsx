...
import {
  unifiedSegmentSearchService,
  UnifiedSearchResult,
  UnifiedSearchOptions,
  UnifiedSearchResponse,
} from "@/lib/unified-segment-search";
...
import { SmartVideoSegmentButton } from "@/components/smart-video-segment-button";
...
export default function UnifiedSearchPage() {
  ...
  const [searchOptions, setSearchOptions] = useState<UnifiedSearchOptions>({
    caseSensitive: false,
    exactMatch: false,
    includePartial: true,
    fuzzyMatch: false,
  ...
  const [stats, setStats] = useState<UnifiedSearchResponse["stats"] | null>(
    null
  );
  ...
  const loadStats = async () => {
    try {
      setError(null);
      const searchStats = await unifiedSegmentSearchService.getSearchStats();
  ...
  const handleSearch = async () => {
    ...
    setError(null);
    try {
      const results = await unifiedSegmentSearchService.search(searchKeyword, {
        ...searchOptions,
      ...
      toast.success(
        `Found ${results.totalResults} results (${results.matchedSubtitles} subtitles, ${results.matchedSegments} segments) from ${results.totalVideos} videos`
      );
    } catch (error) {
    ...
    try {
      ...
      const enhancedSegments = await Promise.all(
        searchResults.results.map(async (result) => {
          try {
            ...
            return {
              subtitle: {
                id: result.subtitleId || result.id,
                text: result.text,
            ...
            console.warn(
              `Could not get video file for ${result.videoId}:`,
              error
            );
            return {
              subtitle: {
      ...
      if (availableSegments.length === 0) {
        ...
        const subtitleContent = searchResults.results
          .map((result, index) => {
            const startTime = formatTimeForSRT(result.startTime);
            const endTime = formatTimeForSRT(result.endTime);
            return `${index + 1}\n${startTime} --> ${endTime}\n${
              result.text
            }\n`;
          })
      ...
      const timestamp = new Date()
        ...
        .slice(0, 19)
        .replace(/:/g, "-");
      const filename = `unified-search-${searchKeyword}-${timestamp}.mp4`;
  ...
  const getKeywordSuggestions = async (partial: string) => {
    ...
    try {
      return await unifiedSegmentSearchService.getKeywordSuggestions(
        partial,
        5
      );
    } catch (error) {
  ...
  const handlePlayPreview = async (result: UnifiedSearchResult) => {
    try {
      // 尝试从存储中获取视频文件
      const { videoStorageService } = await import("@/lib/video-storage");
      const videoFile = await videoStorageService.getVideoFile(result.videoId);
  ...
  const handleRemoveResult = (resultToRemove: UnifiedSearchResult) => {
    if (!searchResults) return;

    const updatedResults = searchResults.results.filter(
      (result) => result.id !== resultToRemove.id
    );
  ...
  const getResultTypeIcon = (type: "subtitle" | "segment") => {
    return type === "segment" ? (
  ...
  const getResultTypeColor = (type: "subtitle" | "segment") => {
    return type === "segment" ? "text-green-600" : "text-blue-600";
  };
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-2">
                Search across subtitles and video segments in one place
              </p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="education-card p-8 mb-8">
          ...
          <div className="relative mb-6">
            ...
            <input
              type="text"
              placeholder="Search across subtitles and video segments..."
              value={searchKeyword}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            <div>
              ...
              <select
                value={searchOptions.segmentStatus}
                onChange={(e) =>
                  setSearchOptions((prev) => ({
                    ...prev,
                    segmentStatus: e.target.value as
                      | "all"
                      | "completed"
                      | "processing"
                      | "failed",
          ...
          <div className="flex items-center space-x-6 mb-6">
            <label className="flex items-center space-x-2">
              <input
                type="checkbox"
                checked={searchOptions.includeSubtitles}
                onChange={(e) =>
                  setSearchOptions((prev) => ({
                    ...prev,
            ...
            <label className="flex items-center space-x-2">
              <input
                type="checkbox"
                checked={searchOptions.includeSegments}
                onChange={(e) =>
                  setSearchOptions((prev) => ({
                    ...prev,
              ...
              />
              <span className="text-sm text-gray-700">Include segments</span>
            </label>
            <label className="flex items-center space-x-2">
              <input
                type="checkbox"
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-gray-600 mt-1">
                  ...
                  {searchResults.matchedSubtitles} subtitles,{" "}
                  {searchResults.matchedSegments} segments) from{" "}
              ...
              <div className="bg-green-50 rounded-lg p-4">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm font-medium text-green-900">
                    {searchResults.matchedSegments} Segments
                  </span>
                </div>
              </div>
              ...
              <div className="bg-indigo-50 rounded-lg p-4">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm font-medium text-indigo-900">
                    {searchResults.totalResults} Total
                  </span>
                </div>
              </div>
            </div>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center space-x-2 mb-2">
                        ...
                        <span
                          className={`text-xs px-2 py-1 rounded-full font-medium ${getResultTypeColor(
                            result.type
                          )}`}
                        >
                        ...
                        {result.type === "segment" && result.status && (
                          <span
                            className={`text-xs px-2 py-1 rounded-full font-medium ${
                      ...
                      <div className="flex items-center space-x-4 text-xs text-gray-500">
                        ...
                        {result.type === "segment" && result.fileSize && (
                          <span className="flex items-center space-x-1">
